(function ($, undefined) {
	$(function () {
				...
				open: function () {
					...
					if ($dialogView.length > 0) {
						...
						if ( typeof $booking_id == 'undefined' ) {
							...
							$html += '<td>Appointment Scheduler</td>';
							$html += '</tr>';
		...
		if ($dialogExtraService.length > 0 && dialog) {
			$dialogExtraService.dialog({
				...
				open: function () {
					...
					$.get("index.php?controller=pjAdmin&action=getExtraService&booking_id=" + $booking_id + $as_pf).done(function (data) {
		...
		if ($dialogEditbookingStatus.length > 0 && dialog) {
			$dialogEditbookingStatus.dialog({
				...
				open: function () {
					...
					$status['arrived'] = 'Client arrived';
					$status['paid'] = 'Client paid';
					$status['no_show_up'] = "Client didn't show up";
		...
		function onChangeEmployee() {
			var $el = $(this),
				$form = $el.closest("form"),
				service_id = $form.find("select[name='employee_id']").find("option:selected").val();
