	...
	public function pjActionFreetime()
	{
		...
		if ($this->isAdmin())
		{
			...
			if ( ( (isset($_GET['date']) && !empty($_GET['date'])) || (isset($_GET['start_ts']) && !empty($_GET['start_ts'])) )  
				...
				$this->set('t_arr', pjAppController::getRawSlotsPerEmployee($_GET['employee_id'], $date, $this->getForeignId()));
				
			} elseif ( count($ef_arr) > 0 ) {
				$this->set('t_arr', pjAppController::getRawSlotsPerEmployee($ef_arr['employee_id'], date("Y-m-d", $ef_arr['start_ts']), $this->getForeignId()));
			
			} elseif ( isset($employee_arr) && count($employee_arr) > 0 ) {
				$this->set('t_arr', pjAppController::getRawSlotsPerEmployee($employee_arr[0]['id'], date("Y-m-d"), $this->getForeignId()));
	...
	public function pjActionEmployeeCustomTime() {
			...
			foreach ($_POST as $k => $v) {
				...
				if ( strpos($k, "id") !== false ) {
					...
					if ( count($_array) > 0 ) {
						...
						$data['date'] = date('Y-m-d', strtotime(date('Y-m') . '-' . $_array[1]));
		...
		if (count($employee_arr) > 0) {
			...
			foreach ($employee_arr as $k => $employee) {
				...
				$employees[$k] = array_merge($employees[$k], $employee_customtime);
			}
