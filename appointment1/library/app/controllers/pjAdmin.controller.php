...
class pjAdmin extends pjAppController
{
	...
	public function getExtraService() {
		...
		if ($this->isXHR() && $this->isLoged()) {
			...
			if ( isset($_POST['extra_id']) && isset($_POST['booking_id']) ) {
				...
				if (isset($_POST['booking_id']) && !empty($_POST['booking_id']))
				{
					...
					{ 
						$pjBookingExtraServiceModel->addBatchRow(array($_POST['booking_id'], '0', $extra_id));
					}
				...
				$this->set('extra_arr', pjExtraServiceModel::factory()
							->select('t1.*')
							->join('pjBookingServiceModel', 't2.booking_id = ' . $_GET['booking_id'], 'left outer')
							->join('pjServiceExtraServiceModel', 't3.extra_id = t1.id AND t3.service_id = t2.service_id', 'left outer')
							->orderBy('t1.name ASC')
							->findAll()
							->getData()
						)
