		...
		if ($this->isXHR())
		{
			...
			if ($booking_id !== false && (int) $booking_id > 0)
			{
				...
				foreach ($summary['services'] as $service)
				{
					...
					if ( isset($_SESSION[ PREFIX . $service['id'] . '_extra' ]) && count($_SESSION[ PREFIX . $service['id'] . '_extra' ]) > 0 ) {
						...
						$pjBookingExtraServiceModel->setBatchFields(array('booking_id', 'service_id', 'extra_id'));
						foreach ( $_SESSION[ PREFIX . $service['id'] . '_extra' ] as $_extra) {
							$pjBookingExtraServiceModel->addBatchRow(array($booking_id, '0', $_extra['id']));
						}
						$pjBookingExtraServiceModel->insertBatch();
					}
					
					if ( isset($_SESSION[ PREFIX . $service['id'] . '_extra' ]) ) unset($_SESSION[ PREFIX . $service['id'] . '_extra' ]);
				}
