	...
	$('form').form();
		$('#category').change(function() {
			...
			$('#loading').show();
					$.ajax({
					  type: "get",
					  async: false,
					  url: "index.php?module=products&view=getSubCategories&prefix=<?php echo PREFIX; ?>",
		...
		$('#product_type').change(function() {
			...
			if ( $product_type == 'services' ) {
				$('.control-group.service').css("display", "none");
				$('#unit').attr( "value", "services");
				$('#size').attr( "value", "");
				$('#alert_quantity').attr( "value", "1");
			} else if ( $product_type == 'giftcard' ) {
				$('.control-group.service').css("display", "none");
				$('#unit').attr( "value", "giftcard");
				$('#size').attr( "value", "");
				$('#alert_quantity').attr( "value", "1");
				
			} else {
				$('.control-group.service').css("display", "block");
				$('#unit').attr( "value", "");
				$('#alert_quantity').attr( "value", "");
			}
		});
...
<?php if($message) { echo "<div class=\"alert alert-error\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button>" . $message . "</div>"; } ?>
	...
	<?php $attrib = array('class' => 'form-horizontal'); echo form_open_multipart("module=products&prefix=" . PREFIX ."&view=edit&id=".$id, $attrib); ?>
...
<div class="control-group">
    <label class="control-label" for="product_type"><?php echo $this->lang->line("product_type"); ?></label>
    <div class="controls">
        <?php
        $ps = array(
        		'products' => $this->lang->line("products"),
        		'services' => $this->lang->line("services"),
        		'giftcard' => $this->lang->line("giftcard")
        );
        echo form_dropdown('product_type', $ps, $product->product_type, 'class="tip chzn-select span4" id="product_type"');
        ?>
    </div>
</div>
