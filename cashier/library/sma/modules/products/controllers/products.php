...
*/
   ...
   /*function barcode($product_code = NULL)
   {
	    ...
	    $data['img'] = "<img src='".$this->config->base_url()."index.php?module=products&amp;prefix=" . PREFIX . "&view=gen_barcode&code={$product_code}' alt='{$product_code}' width='300' />";
   ...
   function barcode($product_code = NULL)
   {
		...
		$data['product_details'] = $this->products_model->getProductByCode($product_code);
	    $data['img'] = "<img src='".$this->config->base_url()."index.php?module=products&amp;prefix=" . PREFIX . "&view=gen_barcode&code={$product_code}' alt='{$product_code}' />";
		$this->load->view('barcode', $data);
   ...
   function product_barcode($product_code = NULL, $height = 60, $width = 210)
   {
	   if($this->input->get('code')){ $product_code = $this->input->get('code'); }
		...
		//$data['product_details'] = $this->products_model->getProductByCode($product_code);
	    return "<img src='".$this->config->base_url()."index.php?module=products&amp;prefix=" . PREFIX . "&view=gen_barcode&code={$product_code}&height={$height}&width={$width}' alt='{$product_code}' />";
    ...
    function sheet()
	{
		...
		$config['base_url'] = base_url().'index.php?module=products&amp;prefix=' . PREFIX . '&view=sheet&category_id='.$category_id;
	...
	function labels()
	{
		...
		$config['base_url'] = base_url().'index.php?module=products&prefix=' . PREFIX . '&view=labels&category_id='.$category_id;
		$config['total_rows'] = $this->products_model->products_count($category_id);
		$config['per_page'] = 10;
		$config['num_links'] = 5;
   ...
   function getwhproducts($warehouse_id = NULL)
   {
	   ...
	   $this->datatables
			...
			->group_by("products.id")
			->add_column("Actions",
			"<center><a id='$4 - $3' href='index.php?module=products&amp;prefix=" . PREFIX . "&view=gen_barcode&code=$3&height=200' title='".$this->lang->line("view_barcode")."' class='barcode tip'><i class='icon-barcode'></i></a>
			<a href='#' onClick=\"MyWindow=window.open('index.php?module=products&amp;prefix=" . PREFIX . "&view=product_details&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=500'); return false;\" class='tip' title='".$this->lang->line("product_details")."'><i class='icon-fullscreen'></i></a>
			...
			<a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=add_damage&product_id=$1&warehouse_id=$5' class='tip' title='".$this->lang->line("add_damage_qty")."'><i class='icon-filter'></i></a>
			<a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=edit&id=$1' class='tip' title='".$this->lang->line("edit_product")."'><i class='icon-edit'></i></a>
			<a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=delete&id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_product') ."')\" class='tip' title='".$this->lang->line("delete_product")."'><i class='icon-trash'></i></a></center>", "productid, image, code, name, wh")
	...
	function getseatchresults($srq = NULL)
   {
	   ...
	   $this->datatables
			...
			->group_by("products.id")
			->add_column("Actions",
			"<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=&amp;prefix=" . PREFIX . "products&view=barcode&code=$3', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=425,height=275'); return false;\">

								<img src='".$this->config->base_url()."smlib/images/bar.png' width='16' height='16' title='".$this->lang->line("view_barcode")."' style='margin:1px;'/>
								</a>
								<a href='#' onClick=\"MyWindow=window.open('index.php?module=products&amp;prefix=" . PREFIX . "&view=product_details&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=500'); return false;\">
								...
								<a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=edit&id=$1'>
								<img src='".$this->config->base_url()."smlib/images/edit.png' width='16' height='16' title='".$this->lang->line("edit_product")."' style='margin:1px;'/>
								</a>
							    <a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=delete&id=$1' onClick='return confirm('". $this->lang->line('alert_x_product') ."')'>
	...
	function add()
	{
		...
		$this->form_validation->set_rules('product_type', $this->lang->line('product_type'), 'xss_clean');
	...
	function getdamageproducts()
   {
	   ...
	   $this->datatables
			...
			$this->datatables->add_column("Actions",
			"<center><a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=edit_damage&product_id=$1&id=$2' class='tip' title='".$this->lang->line("edit_damage_details")."'><i class='icon-edit'></i></a>
			<a href='index.php?module=products&amp;prefix=" . PREFIX . "&view=delete_demage&id=$2' onClick=\"return confirm('". $this->lang->line('alert_x_damage_product') ."')\" class='tip' title='".$this->lang->line("delete_damage_product")."'><i class='icon-trash'></i></a></center>", "productid, did");
	...
	function product_details($id = NULL)
	{
		...
		$data['barcode'] = "<img src='".$this->config->base_url()."index.php?module=products&amp;prefix=" . PREFIX . "&view=gen_barcode&code=".$pr_details->code."' alt='".$pr_details->code."' />";
