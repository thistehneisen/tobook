...
*/
    ...
    function getProductAlerts()
   {
	   ...
	   $this->datatables
			...
			<a href='index.php?module=products&amp;prefix=" . PREFIX . "&amp;view=delete&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_product') ."')\" class='tip' title='".$this->lang->line("delete_product")."'><i class='icon-trash'></i></a></center>", "productid, image, code, name");
		...
		if(!empty($sales)) {
			foreach($sales as $sale){
				...
				<table class='table table-bordered table-hover table-striped table-condensed data' style='margin:0;font-size:10px;margin-top:3px;'>
					...
					<tr>
						<td>".$this->lang->line("Product Tax")."</td>
						<td>". $this->ion_auth->formatMoney($sale->productTax) ."</td>
					</tr>																
				</table>
				<table class='table table-bordered table-hover table-striped table-condensed data' style='margin:0;font-size:10px;margin-top:3px;'>
					<tr>
   ...
   function getCP()
   {
	    if($this->input->get('product')){ $product = $this->input->get('product'); } else { $product = NULL; }
            ...
            if($start_date) {
                ...
                $pp =    "( SELECT pi.product_id, SUM( pi.quantity ) purchasedQty from  ". $this->db->dbprefix('purchases') ." AS p JOIN  ". $this->db->dbprefix('purchase_items') ." AS pi on p.id = pi.purchase_id where
                         p.date >= '{$start_date}' and p.date < '{$end_date}'
                         group by pi.product_id ) PCosts";
                
                $sp = "( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.gross_total ) totalSale from ". $this->db->dbprefix('sales') ." AS s JOIN ". $this->db->dbprefix('sale_items') ." AS si on s.id = si.sale_id where
                       s.date >= '{$start_date}' and s.date < '{$end_date}'
                       group by si.product_id ) PSales";
            } else {
                $pp ="( SELECT pi.product_id, SUM( pi.quantity ) purchasedQty from ". $this->db->dbprefix('purchase_items') ." AS pi group by pi.product_id ) PCosts";
                $sp = "( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.gross_total ) totalSale from ". $this->db->dbprefix('sale_items') ." AS si group by si.product_id ) PSales";
            }
