    ...
    @include ('admin.coupon.tabs')
     {{ Form::open(['route' => ['admin.coupon.campaigns.create', (isset($campaign->id)) ? $campaign->id: null], 'class' => 'form-horizontal well', 'role' => 'form', 'enctype' => 'multipart/form-data']) }}
        ...
        <div class="form-group {{ Form::errorCSS('name', $errors) }}">
            ...
            <div class="col-sm-5">
                {{ Form::text('name', !empty($campaign->id) ? $campaign->name : '', ['class' => 'form-control input-sm', 'id' => 'name']) }}
        ...
        <div class="form-group {{ Form::errorCSS('begin_at', $errors) }}">
            ...
            <div class="col-sm-5">
                {{ Form::text('begin_at', !empty($campaign->id) ? str_standard_to_local($campaign->begin_at) : str_date($today), ['class' => 'form-control input-sm date-picker', 'id' => 'begin_at']) }}
                {{ Form::errorText('begin_at', $errors) }}
            </div>
        </div>
        <div class="form-group {{ Form::errorCSS('expire_at', $errors) }}">
            ...
            <div class="col-sm-5">
                {{ Form::text('expire_at',!empty($campaign->id) ? str_standard_to_local($campaign->expire_at) : '', ['class' => 'form-control input-sm date-picker', 'id' => 'expire_at']) }}
