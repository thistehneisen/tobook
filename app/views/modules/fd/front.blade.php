                ...
                @foreach ($category->deals as $deal)
                    ...
                    <li class="list-group-item">
                        <div class="flashdeal-item text-left">
                            ...
                            <h5>
                                {{ trans('common.price') }}:
                                <span class="orange">{{ $deal->flashDeal->discounted_price }}{{ Settings::get('currency') }}</span>
                                ({{ trans('common.normal') }} {{ $deal->flashDeal->service->price }}{{ Settings::get('currency') }})
                            </h5>
                            ...
                            <p class="text-center"><a data-target="#fd-modal" href="#" data-url="{{ route('fd.view', ['id' => $deal->flash_deal_date_id]) }}" class="btn btn-orange countdown btn-fd" data-date="{{ $deal->expire->format('Y-m-d\TH:i:s') }}"></a></p>
                        </div>
                    </li>
                    @endif
                @endforeach
