        ...
        <div class="as-col">
            <ul>
                <li class="as-col-header">
                    ...
                    <a class="as-weekly-col btn-workshift-switch" data-custom-time-id="{{ $theEmployee->getActiveWorkshift($selectedDate) }}" data-date="{{ str_standard_date($selectedDate) }}" data-employee-id="{{ $theEmployee->id }}" href="#"><i class="fa fa-clock-o as-workshift-switch"></i></a>
                    @endif
                </li>
                @foreach ($workingTimes as $hour => $minutes)
                    @foreach ($minutes as $minuteShift)
                        <?php $slotClass = $theEmployee->getSlotClass(str_standard_date($selectedDate), $hour, $minuteShift); ?>
