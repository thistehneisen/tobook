<li data-booking-date="{{ $selectedDate }}" data-employee-id="{{ $selectedEmployee->id }}" data-start-time="{{ sprintf('%02d:%02d', (int)$hour, $minuteShift) }}" href="#select-action" class="{{ $slotClass }}">
    @if(strpos(trim($slotClass), 'booked') === 0)
        ...
        ?>
        <span class="customer-tooltip" title="{{ $booking->consumer->getNameAttribute() }} {{ $serviceDescription }}"><a class="js-btn-view-booking" href="#" data-booking-id="{{ $booking->id }}" data-employee-id="{{ $selectedEmployee->id }}">{{ $booking->consumer->getNameAttribute() }} {{ $serviceDescription }}</a></span>
