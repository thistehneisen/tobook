         ...
         <table class="table table-bordered">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ Form::select('discount['. $weekday .'][morning]', $discount, 0, ['class'=> 'form-control']) }}</td>
                    @endforeach
                </tr>
                <tr>
                    <td>{{ trans('common.afternoon') }}</td>
                    @foreach ($weekdays as $weekday)
                    <td>{{ Form::select('discount['. $weekday .'][afternoon]', $discount, 0, ['class'=> 'form-control']) }}</td>
                    @endforeach
                </tr>
                <tr>
                    <td>{{ trans('common.evening') }}</td>
                    @foreach ($weekdays as $weekday)
                    <td>{{ Form::select('discount['. $weekday .'][evening]', $discount, 0, ['class'=> 'form-control']) }}</td>
                    @endforeach
                </tr>
            </tbody>
         </table>
