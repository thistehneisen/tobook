...
<head>
    ...
    <?php
		...
		if( isset($_GET['id']) ){
			...
			$sql = "select * from tbl_marketing_auto where marketing_auto = $automationId";
		...
		$sql = "select * from tbl_email_campaign where status = 'DRAFT' or status = 'AUTOMATION'";
...
<body>
	...
	<div class="container">
			<div style="width: 800px;margin:20px auto;">
				<h3 style="color:#e67e22;"><?php echo $MT_LANG['saveAutomation']?></h3>
			</div>
            <div style="width: 800px;margin:20px auto;">
	            <div class="floatright">
	            	<button class="btn-u btn-u-orange" onclick="onSaveAutomation()"><i class="icon-edit"></i>&nbsp;<?php echo $MT_LANG['saveAutomation']?></button>
	            	<button class="btn-u btn-u-blue" onclick="window.location.href='automationList.php'"><i class="icon-list-ul"></i>&nbsp;<?php echo $MT_LANG['automationList']?></button>
	            </div>
				...
				<div class="form-group" id="divEmail" style="<?php if($dataAutomation['type'] != "email" ) echo "display:none;";?>">
                    ...
                    <select class="form-control" id="campaign">
                    	...
                    	<?php for( $i = 0; $i < count($campaignList); $i ++ ){
                    		...
                    		<?php }else{?>
                    		<option value="<?php echo $campaignList[$i]['email_campaign']?>"><?php echo $campaignList[$i]['subject']?></option>
                    		<?php } ?>
                    	<?php } ?>
                    </select>
				</div>
				
				<div class="form-group" id="divSms" style="<?php if($dataAutomation['type'] != "sms" ) echo "display:none;";?>">
                	<label><?php echo $MT_LANG['smsText']?></label>
                    <textarea class="form-control" id="smsText" maxlength="160"><?php echo $dataAutomation['sms_content'];?></textarea>
				</div>
				
				<div class="form-group">
                	<label><?php echo $MT_LANG['pluginType']?></label>
                    <select class="form-control" id="pluginType">
                    	<option value="">Select Plugin Type.</option>
                    	<option value="RB" <?php if($dataAutomation['plan_group_code'] == 'RB') echo "selected";?>><?php echo $MT_LANG['restaurantBookings']?></option>
                    	<option value="TB" <?php if($dataAutomation['plan_group_code'] == 'TB') echo "selected";?>><?php echo $MT_LANG['timeslotBookings']?></option>
                    	<option value="AS" <?php if($dataAutomation['plan_group_code'] == 'AS') echo "selected";?>><?php echo $MT_LANG['appointmentScheduler']?></option>
                    </select>
				</div>
