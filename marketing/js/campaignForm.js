...
function onSelectEmailTemplate( obj ){
	$(obj).parents("#emailTemplateList").eq(0).find("div#emailTemplateItem").removeClass("greyBackground");
...
function onCloseEmailTemplatePopup(){
	$("div#emailTemplateList").find("div#emailTemplateItem").removeClass("greyBackground");
...
function onChooseEmailTemplate( ){
	...
	var ind = $("div#emailTemplateList").find("div#emailTemplateItem").index( obj );
	if( ind == -1 ){
...
function onSaveCampaign( ){
	...
	if( replyEmail == "" ){ alert("Please enter Reply Email."); return; }
	if( replyName == "" ){ alert("Please enter Email Name."); return; }
	...
	$.ajax({
        ...
        dataType : "json",
        type : "POST",
        data : { subject : subject, content : content, ownerId : ownerId, campaignId : campaignId, replyEmail : replyEmail, replyName : replyName },
        success : function(data){
            if(data.result == "success"){
