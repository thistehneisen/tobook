$(document).ready( function(){
	$('#scheduleDate').datepicker({format: 'yyyy-mm-dd', showToday: true});
...
function onDeleteGroup( ){
	...
	for( var i = 0; i < objChkList.size(); i ++ ){
		var groupId = objChkList.eq(i).parents("td").eq(0).find("#groupId").val();
		strGroupIds += groupId + ",";
	}
	if( strGroupIds != "" ){
...
function onSendSMS( ){
	...
	$.ajax({
        ...
        dataType : "json",
        type : "POST",
        data : { ownerId : ownerId, title : title, content : content, groupIds : strGroupIds },
        success : function(data){
            if(data.result == "success"){
            	alert( "SMS will sent.");
            	onCloseSMS( );
...
function onSendEmail( ){
	...
	$.ajax({
        ...
        dataType : "json",
        type : "POST",
        data : { campaignId : campaignId, groupIds : strGroupIds, ownerId : ownerId },
        success : function(data){
            if(data.result == "success"){
