<?php
    ...
    $groupIds = mysql_escape_string($_POST['groupIds']);
    
    // $customerList : customerId : customerId, planGroupCode : planGroupCode, destination : destination
    
    $sql = "select * from tbl_marketing_group_member where marketing_group in ( $groupIds )";
    ...
    for( $i = 0; $i < count( $dataCustomer ); $i ++ ){
    	...
    	$temp['destination'] = $dataCustomer[$i]['phone'];
    	$customerList[] = $temp;
    }    
    ...
    if( $payable == "Y" ){
	    for( $i = 0 ; $i < count( $customerList ); $i ++ ){
	    	...
	    	$sql = "insert into tbl_marketing_history( owner, customer, plan_group_code, destination, messageId, status, marketing_type, marketing_message, created_time, updated_time)
	    			values($ownerId, $customerId, '$planGroupCode', '$destination', '$messageId', '$status', '$marketingType', $marketingMessage, now(), now() )";
	    	$db->queryInsert( $sql );
	    }
