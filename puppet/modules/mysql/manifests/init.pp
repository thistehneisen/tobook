class mysql 
{
    ...
    { 
        "create-default-db":
            unless => "/usr/bin/mysql -uroot -p$mysqlPassword varaa_stag",
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'create database `varaa_stag`;'",
    ...
    { 
        "grant-default-db":
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'grant all on `varaa_stag`.* to `root@localhost`;'",
