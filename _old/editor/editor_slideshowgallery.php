...
<script type="text/javascript" >
		...
		$('#btnGalleryOk').live('click', function()	{
...
if($galleryType == 'slideshow' && $currentPage != ''){		 	// slide show images
	...
	if(sizeof($galleryImage) > 0){
		...
		echo '<div class="edtrgalleryokay"><input type="button" class="popup_orngbtn right " value="Done" id="btnGalleryOk" name="btnGalleryOk"></div>';
	}
...
elseif($galleryType == 'gallery' && $currentPage != ''){	
	...
	echo ' <div id="editor_plaineditor" >
	<div class="popupeditpanel_ftr"><input type="button" name="actUseImage" id="actUseImage" value="Add Image" class="popup_orngbtn right "></div></div>';
    closedir($folder);
}
...
elseif($galleryType == 'settings' && $currentPage != ''){	
	$arrSettingsInfo = $_SESSION['siteDetails'][$currentPage]['datatypes']['slider'][$sliderid]['settings'];
	$slideHeight 	= $arrSettingsInfo['height'];
	$slideWidth 	= $arrSettingsInfo['width'];
	$slideDelay		= $arrSettingsInfo['delay'];
        echo '<div id="settingsresult"></div><br><table    border="0" cellspacing="0" class="pageadd_tbl" cellpadding="0">
<tr><td width="20%">Width </td><td width="30%"><input class="textbox_style2" type="text" name="width" value="'.$slideWidth.'" size="10" ></td><td width="50%">width of the slider<td></tr>
