...
function onDeleteAutomation() {
    ...
    for ( var i = 0; i < objChkList.size(); i++) {
        var automationId = objChkList.eq(i).parents("td").eq(0).find(
                "#automationId").val();
        strAutomationIds += automationId + ",";
    }
    if (strAutomationIds != "") {
        strAutomationIds = strAutomationIds.substr(0,
                strAutomationIds.length - 1);
    } else {
    ...
    $.ajax({
        ...
        type : "POST",
        data : {
            automationIds : strAutomationIds
        },
...
function onDuplicate() {
    ...
    $.ajax({
        ...
        type : "POST",
        data : {
            duplicateCampaignId : duplicateCampaignId,
            ...
            .ajax({
                ...
                success : function(data) {
                    if (data.result == "success") {
                        ...
                        $("#cntUnsubscribes")
                                .text(data.statistics.unsubscribes);
