...
if (isset($tpl['arr']))
{
	if (is_array($tpl['arr']))
	{
		...
		{
			?>
			<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate&amp;as_pf=<?php echo $as_pf; ?>" method="post" class="form pj-form">
			...
			for ($i = 0; $i < $count; $i++)
			{
				...
				<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
					<td>
					<?php if ($tpl['arr'][$i]['key'] == 'o_reminder_sms_send_address') {
						echo $tpl['arr'][$i]['label'];
					} else  __('opt_' . $tpl['arr'][$i]['key']); ?>
					</td>
					<td class="align_top">
						<?php
