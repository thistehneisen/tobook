		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			if ( isset($_GET['tyle']) && $_GET['tyle'] == 'freetime' ) {
				$pjEmployeeFreetimeModel = pjEmployeeFreetimeModel::factory()
						->select('t1.*, t2.*')
                        ->where('t1.owner_id', $owner_id)
						->join('pjMultiLang', "t2.model='pjEmployee' AND t2.foreign_id=t1.employee_id", 'left inner');
		...
		if ($this->isAdmin())
		{
			...
			if (isset($_POST['employee_update']) && isset($_POST['id']) && (int) $_POST['id'] > 0)
			{
				...
				foreach ($locale_arr as &$item)
				{
					...
					$lp_arr[$item['id']."_"] = 'gb.png'; //Hack for jquery $.extend, to prevent (re)order of numeric keys in object
				}
				$this->set('lp_arr', $locale_arr);
