...
class pjFrontEnd extends pjFront
{
		...
		if ($this->isXHR())
		{
			...
			if ($booking_id !== false && (int) $booking_id > 0)
			{
				...
				$booking_arr = $pjBookingModel
					...
					->join('pjMultiLang', "t3.model='pjCountry' AND t3.field='name' AND t3.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t4.model='pjCalendar' AND t4.field='confirm_subject_client' AND t4.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t5.model='pjCalendar' AND t5.field='confirm_tokens_client' AND t5.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t6.model='pjCalendar' AND t6.field='confirm_subject_admin' AND t6.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t7.model='pjCalendar' AND t7.field='confirm_tokens_admin' AND t7.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t8.model='pjCalendar' AND t8.field='confirm_subject_employee' AND t8.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t9.model='pjCalendar' AND t9.field='confirm_tokens_employee' AND t9.owner_id = t1.owner_id", 'left outer')
    ...
    public function pjActionLoadAjax () {
        ...
        if ( isset($_GET['layout']) && (int) $_GET['layout'] == 2 ) {
	    	...
	    	} elseif (isset($_GET['employee_id']) && ((int) $_GET['employee_id'] > 0 || $_GET['employee_id'] == 'all') ) {
