...
<script>
             $(document).ready(function() {
				$('#fileData').dataTable( {
					...
					'bProcessing'    : true,
					'bServerSide'    : true,
					'sAjaxSource'    : '<?php echo base_url(); ?>index.php?module=inventories&prefix=<?php echo PREFIX; ?>&view=getwhinv&warehouse_id=<?php echo $warehouse_id; ?>',
...
<?php $links = '<li><a href="index.php?module=inventories&prefix=' . PREFIX . '">'.$this->lang->line("all_warehouses").'</a></li><li class="divider"></li>';
	foreach($warehouses as $warehouse) {
		if($warehouse->id == $warehouse_id) {
			echo $warehouse->name;
		} else {
			$links .= "<li><a href='index.php?module=inventories&prefix=" . PREFIX . "&view=warehouse&warehouse_id=".$warehouse->id."'>".$warehouse->name."</a></li>";
		}
	} 
	?>
...
<ul class="dropdown-menu">
	...
	<a href="<?php echo site_url('module=inventories&view=add&prefix=' . PREFIX);?>" class="btn btn-primary"><?php echo $this->lang->line("add_purchase"); ?></a> <div class="btn-group dropup" style="margin-left: 25px;">
<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
<?php 
	foreach($warehouses as $warehouse) {
		if($warehouse->id == $warehouse_id) {
