...
*/
   ...
   function giftcard()
   {
	   	...
	   	$meta['page_title'] = $this->lang->line("giftcard");
	   	$data['page_title'] = $this->lang->line("giftcard");
	   	$this->load->view('commons/header', $meta);
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			...
			->add_column("Actions", 
			"<center>			<a class=\"tip\" title='".$this->lang->line("edit_customer")."' href='index.php?module=customers&amp;prefix=" . PREFIX . "&amp;view=edit&amp;id=$1'><i class=\"icon-edit\"></i></a>
							    <a class=\"tip\" title='".$this->lang->line("delete_customer")."' href='index.php?module=customers&amp;prefix=" . PREFIX . "&amp;view=delete&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_customer') ."')\"><i class=\"icon-remove\"></i></a></center>", "id")
			->unset_column('id');
   ...
   function getgiftcardajax()
   {
   
	   	$this->load->library('datatables');
	   	$this->datatables
	   	->select("name, giftcard_code, giftcard, FROM_UNIXTIME(giftcard_code)")
	   	->from("customers");
   ...
   function giftcard_number_call(){
   		
		if ( isset($_GET['giftcard_code']) ) {
			$giftcard = $this->customers_model->getCustomerByCode($_GET['giftcard_code']);
			
			if ( $giftcard == false ) {
				echo 'Undefined';
			} else echo $giftcard->giftcard;
		}
