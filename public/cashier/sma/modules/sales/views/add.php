<link
	href="<?php echo $this->config->base_url(); ?>assets/css/datepicker.css"
	rel="stylesheet">
<style type="text/css">
.table th {
	text-align: center;
...
<script
	src="<?php echo $this->config->base_url(); ?>assets/js/validation.js"></script>
        ...
        $('#code').keydown(function(e) {
            ...
            if (e.keyCode == 13) {
                ...
                $.ajax({
                    type: "get",
                    async: false,
                    url: "<?php echo $this->config->base_url(); ?>index.php?module=sales&view=scan_item&prefix=<?php echo PREFIX; ?>",
...
<?php if (TAX2) { ?>$('#ttax2').val(total_tax2.toFixed(2));<?php } ?>
    							...
    							if ( $('#gtotal').val().length > 0 ) {
    								twt = parseFloat($('#gtotal').val());
    				            } 
...
<?php if (TAX2) { ?>$('#ttax2').val(total_tax2.toFixed(2));<?php } ?>
                        ...
                        $("#name").autocomplete({
                            source: function(request, response) {
                                $.ajax({url: "<?php echo site_url('module=sales&view=suggestions&prefix=' . PREFIX); ?>",
                            ...
                            $(this).removeClass('ui-autocomplete-loading');
                                ...
                                $.ajax({
                                    type: "get",
                                    async: false,
                                    url: "<?php echo $this->config->base_url(); ?>index.php?module=sales&view=add_item&prefix=<?php echo PREFIX; ?>",
...
<?php if (TAX2) { ?>$('#ttax2').val(total_tax2.toFixed(2));<?php } ?>
                        ...
                        $("#codes").autocomplete({
                            source: function(request, response) {
                                $.ajax({url: "<?php echo site_url('module=sales&view=codeSuggestions&prefix=' . PREFIX); ?>",
                        ...
                        $("form").submit(function() {
							...
							if ( p_val == 'GC' || p_val == 'cashGC' || p_val == 'CCGC' ) {
							   ...
							   if ( giftcard > giftcard_max ) {
									bootbox.alert("Your giftcard value max is " + giftcard_max);
									return false;
							   }
...
<?php } ?>
                        ...
                        $('#addSale_form').on('blur', '#giftcard_number', function(){ 
                         	if($('#giftcard_number').val().length != 0 ) {
                        ...
                        $('#addSale_form').on('blur', '#paid-giftcard-amount', function(){ $('#paid_giftcard_val').val($(this).val()); });
                        $('#addSale_form').on('blur', '.pa', function(){ $('#paid_val').val($(this).val()); });
                        $('#addSale_form').on('change', '#pcc', function(){ $('#cc_no_val').val($(this).val()); });
                        $('#addSale_form').on('change', '#pcc_holder', function(){ $('#cc_holder_val').val($(this).val()); });
                        $('#addSale_form').on('change', '#cheque_no', function(){ $('#cheque_no_val').val($(this).val()); });
                        ...
                        $('#addSale_form').on('blur', '#paid-amount, #paid-giftcard-amount, #pcc', function(){
...
echo form_open ( "module=sales&view=add&prefix=" . PREFIX, $attrib );
?>
...
if (TAX1) {
            ...
            <th class="span2"><?php echo $this->lang->line("quantity"); ?></th>
				...
				<th style="width: 20px;"><i class="icon-trash"
...
<?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : ""), 'class="input-block-level" id="note" style="margin-top: 10px; height: 100px;"'); ?> 
	...
	<div class="span5">
		...
		<div class="control-group inverse"
...
<?php } ?>
		...
		<div class="control-group inverse paidby" style="margin-bottom: 5px;">
			<label class="control-label" style="cursor: default;"><?php echo $this->lang->line("paid_by"); ?></label>
			<div class="controls">
		...
		<div class="pcash">
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="paid-amount" class="pa input-block-level" />
				</div>
			</div>
		</div>
		<div class="pcc" style="display: none;">
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="pcc" value="" class="input-block-level"/>
				</div>
			</div>
		...
		<div class="pcheque" style="display: none;">
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="cheque_no" class="input-block-level"/>
				</div>
			</div>
		</div>
		...
		<input id="rpaidby" type="hidden" value="cash" name="rpaidby"> 
		<input id="paid_val" type="hidden" value="" name="paid_val"> 
		...
		<input id="cc_no_val" type="hidden" value="" name="cc_no_val"> 
		<input id="cc_holder_val" type="hidden" value="" name="cc_holder_val"> 
		<input id="cheque_no_val" type="hidden" value="" name="cheque_no_val">

	</div>
</div>

<div class="control-group">
	<div class="controls"><?php echo form_submit('submit', $this->lang->line("submit"), 'class="btn btn-primary" style="padding: 6px 15px;"'); ?></div>
</div>
<?php echo form_close(); ?> 
