...
//index or inventories page
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			->select("sales.id as sid, date, reference_no, biller_name, customer_name, round(total_tax * ( 100 - inv_discount ) / 100) as total_tax, total_tax2, round(total * ( 100 - inv_discount ) / 100) as total, internal_note")
			->from('sales');
			$this->datatables->add_column("Actions", 
			"<center>"
			// ."<a href='#' title='$2' class='tip' data-html='true'><i class='icon-folder-close'></i></a>"
			."<a href='#' onClick=\"MyWindow=window.open('index.php?module=sales&amp;prefix=" . PREFIX . "&view=view_invoice&id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='".$this->lang->line("view_invoice")."' class='tip'><i class='icon-fullscreen'></i></a> 
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&view=add_delivery&id=$1' title='".$this->lang->line("add_delivery_order")."' class='tip'><i class='icon-road'></i></a>
			...
			<a href='#' onClick=\"window.open('index.php?module=pos&amp;prefix=" . PREFIX . "&amp;view=view_invoice&amp;id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false; \" title='Print Receipt' class='tip'><i class='icon-print'></i></a>
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&amp;view=delete&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_invoice') ."')\" title='".$this->lang->line("delete_invoice")."' class='tip'><i class='icon-trash'></i></a></center>", "sid, internal_note")
   ...
   function getwhinv($warehouse_id = NULL)
   {
	   ...
	   $this->datatables
			->select("id, date, reference_no, biller_name, customer_name, round(total_tax * ( 100 - inv_discount ) / 100) as total_tax, total_tax2, round(total * ( 100 - inv_discount ) / 100) as total")
			->from('sales')
			...
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&amp;view=edit&amp;id=$1' title='".$this->lang->line("edit_invoice")."' class='tip'><i class='icon-edit'></i></a>
			<a href='#' onClick=\"window.open('index.php?module=pos&amp;prefix=" . PREFIX . "&amp;view=view_invoice&amp;id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false; \" title='Print Receipt' class='tip'><i class='icon-print'></i></a>					
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&amp;view=delete&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_invoice') ."')\" title='".$this->lang->line("delete_invoice")."' class='tip'><i class='icon-trash'></i></a></center>", "id")
   ...
   function add()
   {
	   $groups = array('purchaser', 'viewer');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$paid_giftcard_val = $this->input->post ( 'paid_giftcard_val' );
			
			$customer_data = array();
			
			foreach ($customer_details as $customer_key => $customer_value) {
			...
			$inv_total_no_tax = 0;
				...
				for($i=1; $i<=500; $i++){
					if( $this->input->post($quantity.$i) && $this->input->post($product.$i) && $this->input->post($unit_price.$i) ) {
						...
						if ( $product_details->product_type == 'giftcard' ) {
						
							$customer_data['giftcard'] = $customer_data['giftcard'] + (($this->input->post ( $quantity . $i )) * ($this->input->post ( $unit_price . $i )));
						
							if ( !isset($customer_data['giftcard_code']) || $customer_data['giftcard_code'] < strtotime('2014-1-1 00:00:00') ) {
								$customer_data['giftcard_code'] = time();
							}
						}
				...
				$this->sales_model->updateCustomer($customer_id, $customer_data);
		...
		/*	if(!empty($inv_product_code)) {	 
			...
			$keys = array("product_id", "product_type", "product_code","product_name","product_unit", "tax_rate_id", "tax","quantity","unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id");
		
			$items = array();
			foreach ( array_map(null, $product_id, $product_type, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID) as $key => $value ) {
			...
			if ( isset ($discount_tax) && $discount_tax == 1 ) {
			
				$inv_total_no_tax = $inv_total_no_tax * (100 - $ds) / 100;
				$total_tax = $total_tax * (100 - $ds) / 100;
				$val_tax2 = $val_tax2 * (100 - $ds) / 100;
			
			} elseif ( isset ($discount_tax) && $discount_tax == 2 ){
			
				$ds = $ds*100/($inv_total_no_tax + $total_tax + $val_tax2);
				$inv_total_no_tax = $inv_total_no_tax * (100 - $ds) / 100;
				$total_tax = $total_tax * (100 - $ds) / 100;
				$val_tax2 = $val_tax2 * (100 - $ds) / 100;
			}
			...
			$saleDetails = array('reference_no' => $reference_no,
					...
					'paid_giftcard_val' => $paid_giftcard_val,
		...
		{ 
			...
			if ( $customer_details_code = $this->sales_model->getCustomerByCode ( $this->input->post ( 'paid_giftcard_munber' ) )) {
				
				$customer_data = array();
			
				foreach ($customer_details_code as $customer_key => $customer_value) {
					$customer_data[$customer_key] = $customer_value;
				}
			}
			
			if ( ($paid_by == 'GC' || $paid_by == 'cashGC' || $paid_by == 'CCGC') &&
			( isset($customer_data['giftcard_code']) && $customer_data['giftcard_code'] > strtotime('2014-1-1 00:00:00') ) ) {
			
				$customer_data['giftcard'] = $customer_data['giftcard'] - $paid_giftcard_val;
					
			}
				
			$this->sales_model->updateCustomer($customer_data['id'], $customer_data);
			
			if( $saleID = $this->sales_model->addSale($saleDetails, $items, $warehouse_id) ) {		
				$this->session->set_flashdata('success_message', $this->lang->line("sale_added"));
				
				echo '<script type="text/javascript" language="javascript">
						window.open("index.php?module=pos&prefix=' . PREFIX . '&view=view_invoice&id=' . $saleID .'",  "_blank");
						window.location.replace("'. site_url("module=sales&prefix=" . PREFIX) .'"); 
						</script>';
...
function quote_to_invoice()
   ...
   function getdeliveries()
   {
	   ...
	   $this->datatables
			...
			->from('deliveries');
			$this->datatables->add_column("Actions", 
			"<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=sales&prefix=" . PREFIX . "&view=view_delivery&id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='".$this->lang->line("view_delivery")."' class='tip'><i class='icon-fullscreen'></i></a> 
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&amp;view=edit_delivery&amp;id=$1' title='".$this->lang->line("edit_delivery")."' class='tip'><i class='icon-edit'></i></a>
			<a href='index.php?module=sales&amp;prefix=" . PREFIX . "&amp;view=delete_delivery&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_delivery') ."')\" title='".$this->lang->line("delete_delivery")."' class='tip'><i class='icon-trash'></i></a></center>", "id")
