...
class Sales_model extends CI_Model
{
	...
	{
		$myQuery = "SELECT date_format( date, '%b' ) as month, SUM( total ) as purchases FROM  ". $this->db->dbprefix('purchases') ." AS purchases WHERE date >= date_sub( now( ) , INTERVAL 12 MONTH ) GROUP BY date_format( date, '%b' ) ORDER BY date_format( date, '%m' ) ASC";
	...
	{
		$myQuery = "SELECT date_format( date, '%b' ) as month, SUM( total ) as sales FROM  ". $this->db->dbprefix('sales') ." AS sales WHERE in_type = 'real' AND date >= date_sub( now( ) , INTERVAL 12 MONTH ) GROUP BY date_format( date, '%b' ) ORDER BY date_format( date, '%m' ) ASC";
		$q = $this->db->query($myQuery);
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
