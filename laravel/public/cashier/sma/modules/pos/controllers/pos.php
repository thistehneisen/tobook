...
class Pos extends MX_Controller {
	...
	/*
	 * | ----------------------------------------------------- | PRODUCT NAME: 	SIMPLE POS | ----------------------------------------------------- | AUTHER:			MIAN SALEEM | ----------------------------------------------------- | EMAIL:			saleem@tecdiary.com | ----------------------------------------------------- | COPYRIGHTS:		RESERVED BY TECDIARY IT SOLUTIONS | ----------------------------------------------------- | WEBSITE:			http://tecdiary.net | ----------------------------------------------------- | | MODULE: 			POS | ----------------------------------------------------- | This is sales module controller file. | -----------------------------------------------------
	 */
	...
	function index() {
		...
		if ($this->form_validation->run () == true) {
			...
			for($i = 1; $i <= 500; $i ++) {
				...
				if ($this->input->post ( $quantity . $i ) && $this->input->post ( $product . $i ) && $pUnitPrice) {
					...
					if (TAX1) {
						...
						if ($taxType == 1 && $taxRate != 0) {
							$item_tax = (($this->input->post ( $quantity . $i )) * ($pUnitPrice) * $taxRate / 100);
							$val_tax [] = $item_tax;
						} else {
							$item_tax = $taxRate;
							$val_tax [] = $item_tax;
					...
					if (DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 2) {
						...
						if ($dsType == 1 && $ds != 0) {
							$val_ds [] = (($this->input->post ( $quantity . $i )) * ($pUnitPrice) * $ds / 100);
						} else {
						...
						if ($dsType == 1 && $ds != 0) {
							$val_ds [] = (((($this->input->post ( $quantity . $i )) * ($pUnitPrice) + $item_tax) * $ds) / 100);
						} else {
							$val_ds [] = $ds * ($this->input->post ( $quantity . $i ));
						}
					...
					$inv_unit_price [] = $pUnitPrice;
					$inv_gross_total [] = (($this->input->post ( $quantity . $i )) * ($pUnitPrice));
					$product_id [] = $product_details->id;
					$product_type [] = $product_details->product_type;
					$product_name [] = $product_details->name;
					$product_code [] = $product_details->code;
					$product_unit [] = $product_details->unit;
					$inv_total_no_tax += (($this->input->post ( $quantity . $i )) * ($pUnitPrice));
				
					if ( $product_details->product_type == 'giftcard' ) {
						
						$customer_data['giftcard'] = $customer_data['giftcard'] + (($this->input->post ( $quantity . $i )) * ($pUnitPrice));
			...
			$this->pos_model->updateCustomer($customer_id, $customer_data);
			...
			/*
			 * if(!empty($inv_product_code)) { foreach($inv_product_code as $pr_code){ $product_details = $this->pos_model->getProductByCode($pr_code); $product_id[] = $product_details->id; $product_name[] = $product_details->name; $product_code[] = $product_details->code; $product_unit[] = $product_details->unit; } }
			 */
		...
		if ($this->form_validation->run () == true && ! empty ( $items )) {
			...
			if ( $customer_details_code = $this->pos_model->getCustomerByCode ( $this->input->post ( 'paid_giftcard_munber' ) )) {
			...
			$this->pos_model->updateCustomer($customer_data['id'], $customer_data);
			...
			if ($suspend) {
				...
				if ($saleID = $this->pos_model->addSale ( $saleDetails, $items, $warehouse_id, $did )) {
					...
					echo '<script type="text/javascript" language="javascript">
						...
						window.location.replace("'. site_url("module=pos&prefix=" . PREFIX) .'"); 
						</script>';
	...
	function tax_rates() {
		...
		if ($old_id) {
			...
			$tax = array (
					'old_tax_rate' => $old_tax_rate_details->rate,
	...
	function discounts() {
		...
		if ($old_id) {
			...
			$ds = array (
					'old_discount' => $old_discount_details->discount,
	...
	function ajaxproducts($category_id = NULL) {
		...
		$config = array ();
		$config ["base_url"] = base_url () . "index.php?module=pos&view=ajaxproducts&prefix=" . PREFIX;
	...
	function view_invoice() {
		...
		if( $inv->inv_discount < 0 ) $inv->inv_discount = 0; 
		$data ['items'] = $this->pos_model->getInvoiceBySaleID ( $sale_id );
	...
	function getSuspendedSales() {
		...
		$this->datatables->select ( "suspended_bills.id as id, date, customers.name, count, tax1, tax2, discount, total" )->from ( 'suspended_bills' )->join ( 'customers', 'customers.id=suspended_bills.customer_id', 'left' )->group_by ( 'suspended_bills.id' );
		$this->datatables->add_column ( "Actions", "<center><a class=\"tip\" title='" . $this->lang->line ( "add_to_pos" ) . "' href='index.php?module=pos&amp;prefix=" . PREFIX . "&amp;suspend_id=$1'><i class=\"icon-plus-sign\"></i></a> <a class=\"tip\" title='" . $this->lang->line ( "delete_suspended_sale" ) . "' href='index.php?module=pos&amp;view=delete&amp;id=$1' onClick=\"return confirm('" . $this->lang->line ( 'alert_x_sale' ) . "')\"><i class=\"icon-trash\"></i></a></center>", "id" )->
	...
	function email_receipt($sale_id = NULL) {
		...
		if ($this->email->send ()) {
			echo json_encode ( array (
					'msg' => $this->lang->line ( "email_sent" ) 
			) );
		} else {
			echo json_encode ( array (
					'msg' => $this->lang->line ( "email_failed" ) 
			) );
		}
	}
}
