...
<?php if($success_message) { echo "<div class=\"alert alert-success\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button>" . $success_message . "</div>"; } ?>
	...
	<table id="fileData" class="table table-bordered table-hover table-striped" style="margin-bottom: 5px;">
		...
		<?php foreach ($users as $user):?>
			<tr>
                ...
                <?php echo '
                <a class="tip" title="'.$this->lang->line("edit_user").'" href="index.php?module=auth&view=edit_user&prefix=' . PREFIX . '&id=' . $user->id . '"><i class="icon-edit"></i></a> ';
								if ($this->ion_auth->in_group('owner')) {
								echo '<a class="tip" title="'.$this->lang->line("delete_user").'" href="index.php?module=auth&view=delete_user&prefix=' . PREFIX . '&id=' . $user->id . '" onClick="return confirm(\''. $this->lang->line('alert_x_user') .'\');"><i class="icon-trash"></i></a>
                '; }  ?></td>
			</tr>
		<?php endforeach;?>
        </tbody>
	...
	<p><a href="<?php echo site_url('module=auth&view=create_user&prefix=' . PREFIX);?>" class="btn btn-primary"><?php echo $this->lang->line("add_user"); ?></a></p>
