...
class CI_DB_active_record extends CI_DB_driver {
    ...
    public function set_insert_batch($key, $value = '', $escape = TRUE)
    {
        foreach ($key as &$row) {
            if ($this->getByUser === true) {
                @session_start();
                $row['owner_id'] = (int) $_SESSION['owner_id'];
            }
        }
    ...
    function insert($table = '', $set = NULL)
    {
        ...
        if ( ! is_null($set))
        {
            if ($this->getByUser === true) {
                @session_start();
                $set['owner_id'] = (int) $_SESSION['owner_id'];
            }
    ...
    public function update($table = '', $set = NULL, $where = NULL, $limit = NULL)
    {
        ...
        if ($this->getByUser === true) {
            @session_start();
            $this->where('owner_id', (int) $_SESSION['owner_id']);
        }
    ...
    public function enableGetByUser()
    {
        $this->getByUser = true;
        return $this;
    }

    public function disableGetByUser()
    {
    ...
    {
        /**
         * Get the current login user and limit to his/her data only
         */
        @session_start();
        if ($this->getByUser === true && isset($_SESSION['owner_id'])) {
            $table = str_replace($this->dbprefix, '', $this->ar_from[0]);
            $table = str_replace('`', '', $table);
            $this->where($table.'.owner_id', (int) $_SESSION['owner_id']);
        }
