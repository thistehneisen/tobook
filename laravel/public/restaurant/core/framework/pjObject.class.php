 ...
 */
	...
	{
		$arr = array();
		if (!is_array($fk_value))
		{
			$operator = '=';
			$fk_value = "'$fk_value'";
		} else {
			$operator = 'IN';
			$fk_value = "('".join("','", $fk_value)."')";
		}
		$r = mysql_query(sprintf("SELECT `%1\$s` FROM `%2\$s` WHERE `%3\$s` %4\$s %5\$s",
			$associationKey, $table, $foreignKey, $operator, $fk_value)) or die(mysql_error());
