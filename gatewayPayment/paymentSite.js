        ...
        success : function(data){
            if(data.result == "success"){
                ...
                if (data.couponType==currencyNow){
                    $("#coupon_discount").find(".description").text("Discount");
                    $("#coupon_discount").find(".price").text("-"+(data.coupontAmount/100)+" "+currencyNow); 
                    discount=data.coupontAmount/100;
                } else{
                    $("#coupon_discount").find(".description").text("Discount");
                    $("#coupon_discount").find(".price").text("-"+(data.coupontAmount)+"%");
                    discount=price*(data.coupontAmount/100);
                }
                $("#subscription_subtotal").find(".price").text("€ "+(price-discount)+" "+currencyNow);    
                $("#order_total").find(".price").text("€ "+(price-discount)+" "+currencyNow); 
                $("#invalidCoupon").hide();
                $("#validCoupon").val("Y");
            }else{
