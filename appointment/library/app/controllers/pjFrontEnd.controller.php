			...
			if ($booking_id !== false && (int) $booking_id > 0)
			{
				...
				$booking_arr = $pjBookingModel
					...
					->select('t1.*, t1.id AS `booking_id`, t2.email AS `admin_email`, t3.content AS `country_name`,
						...
						t8.content AS `confirm_subject_employee`, t9.content AS `confirm_tokens_employee`,
                        t11.length AS `extra_length`, t11.price AS `extra_price`')
					->join('pjUser', 't2.owner_id= t1.owner_id', 'left outer')
					->join('pjMultiLang', "t3.model='pjCountry' AND t3.foreign_id=t1.c_country_id AND t3.field='name' AND t3.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t4.model='pjCalendar' AND t4.foreign_id=t1.calendar_id AND t4.field='confirm_subject_client' AND t4.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t5.model='pjCalendar' AND t5.foreign_id=t1.calendar_id AND t5.field='confirm_tokens_client' AND t5.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t6.model='pjCalendar' AND t6.foreign_id=t1.calendar_id AND t6.field='confirm_subject_admin' AND t6.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t7.model='pjCalendar' AND t7.foreign_id=t1.calendar_id AND t7.field='confirm_tokens_admin' AND t7.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t8.model='pjCalendar' AND t8.foreign_id=t1.calendar_id AND t8.field='confirm_subject_employee' AND t8.owner_id = t1.owner_id", 'left outer')
					->join('pjMultiLang', "t9.model='pjCalendar' AND t9.foreign_id=t1.calendar_id AND t9.field='confirm_tokens_employee' AND t9.owner_id = t1.owner_id", 'left outer')
                    ->join('pjBookingExtraService', "t10.booking_id = t1.id", 'left outer')
                    ->join('pjExtraService', "t11.id = t10.extra_id", 'left outer')
	...
	public function pjActionConfirmAuthorize()
	{
		...
		if (!empty($invoice_arr))
		{
			...
			$booking_arr = $pjBookingModel
				...
				->join('pjMultiLang', "t4.model='pjCountry' AND t4.foreign_id=t1.c_country_id AND t4.owner_id=t1.owner_id AND t4.field='name'", 'left outer')
				->join('pjMultiLang', "t5.model='pjCalendar' AND t5.foreign_id=t1.calendar_id AND t5.owner_id=t1.owner_id AND t5.field='payment_subject_client'", 'left outer')
				->join('pjMultiLang', "t6.model='pjCalendar' AND t6.foreign_id=t1.calendar_id AND t6.owner_id=t1.owner_id AND t6.field='payment_tokens_client'", 'left outer')
				->join('pjMultiLang', "t7.model='pjCalendar' AND t7.foreign_id=t1.calendar_id AND t7.owner_id=t1.owner_id AND t7.field='payment_subject_admin'", 'left outer')
				->join('pjMultiLang', "t8.model='pjCalendar' AND t8.foreign_id=t1.calendar_id AND t8.owner_id=t1.owner_id AND t8.field='payment_tokens_admin'", 'left outer')
				->join('pjMultiLang', "t9.model='pjCalendar' AND t9.foreign_id=t1.calendar_id AND t9.owner_id=t1.owner_id AND t9.field='payment_subject_employee'", 'left outer')
			...
			if (!empty($booking_arr))
			{
				...
				if ($response !== FALSE && $response['status'] === 'OK')
				{
					...
					$booking_arr['bs_arr'] = pjBookingServiceModel::factory()
						...
						->join('pjMultiLang', "t5.model='pjService' AND t5.foreign_id=t1.service_id AND t5.field='name' AND t5.owner_id=t2.owner_id", 'left outer')
						->join('pjMultiLang', "t6.model='pjEmployee' AND t6.foreign_id=t1.employee_id AND t6.field='name' AND t6.owner_id=t2.owner_id", 'left outer')
	...
	public function pjActionConfirmPaypal()
	{
		...
		if (!empty($invoice_arr))
		{
			...
			$booking_arr = $pjBookingModel
				...
				->join('pjMultiLang', "t8.model='pjCalendar' AND t8.foreign_id=t1.calendar_id AND t8.owner_id=t1.owner_id AND t8.field='payment_tokens_admin'", 'left outer')
				->join('pjMultiLang', "t9.model='pjCalendar' AND t9.foreign_id=t1.calendar_id AND t9.owner_id=t1.owner_id AND t9.field='payment_subject_employee'", 'left outer')
				->join('pjMultiLang', "t10.model='pjCalendar' AND t10.foreign_id=t1.calendar_id AND t10.owner_id=t1.owner_id AND t10.field='payment_tokens_employee'", 'left outer')
				->where('t1.uuid', $invoice_arr['order_id'])
				->limit(1)
				->findAll()
				->getData();
			if (!empty($booking_arr))
			{
				...
				if ($response !== FALSE && $response['status'] === 'OK')
				{
					...
					$booking_arr['bs_arr'] = pjBookingServiceModel::factory()
						...
						->join('pjService', 't3.id=t1.service_id', 'inner')
						->join('pjEmployee', 't4.id=t1.employee_id', 'inner')
						->join('pjMultiLang', "t5.model='pjService' AND t5.foreign_id=t1.service_id AND t5.field='name'", 'left outer')
						->join('pjMultiLang', "t6.model='pjEmployee' AND t6.foreign_id=t1.employee_id AND t6.field='name'", 'left outer')
						->where('t1.booking_id', $booking_arr['id'])
						->findAll()
						->getData();
