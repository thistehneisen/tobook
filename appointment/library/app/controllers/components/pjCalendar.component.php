...
class pjCalendar
{
    ...
    private $dayNames = array("Su", "Ma", "Ti", "Ke", "To", "Pe", "La");
    ...
    protected $classPending = "abCalendarPending";
    protected $classReserved = "abCalendarReserved";
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
        	    ...
        	    if ($d < 1 || $d > $daysInMonth) {
        	    	...
        	    	$class = $this->onBeforeShow($timestamp, $iso, $today, $current, $year, $month, $d);
        	    	$s .= '<td
        	    		class="'.$this->classCell.' '.$class.'"
        	    		data-iso="'.$iso.'"
        	    		data-time="'.$timestamp.'"><div class="'.$this->classDateOuter.'"><p class="'.$this->classDateInner.'">'.$d.'</p></div></td>';
        	    }
        	    $d++;
    	    }
    	    if ($this->getWeekNumbers())
