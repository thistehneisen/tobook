...
<html>
	<head>
		...
		<?php 
	        ...
	        $h = '.asContainer .asServiceName, .asContainer .asEmployeeName{ ';
		if ( isset($tpl['style'][0]['headings']) && !empty($tpl['style'][0]['headings']) ) {
			$h .= 'color: '. $tpl['style'][0]['headings'] .';';
		}
                $h.= ' }';
		$s = 'body { ';
		...
		echo '<style>' . $s . $font . $color . $h. $message . '</style>';
		?>
	</head>
	<body>
		<header>
