<?php
	...
	$table_prefix = $_GET['username'].'_';
	
	$sql = file_get_contents(dirname(__FILE__) . '/library/app/config/database.sql');
	...
	/* $sql_opt = "INSERT INTO `".$table_prefix."ts_booking_options` (`calendar_id`, `key`, `tab_id`, `group`, `value`, `description`, `label`, `type`, `order`) VALUES
...
(1, 'reminder_subject', 6, NULL, 'Booking Reminder', 'Email Reminder subject', NULL, 'string', 3);";
		...
		$sql = "insert into ".$table_prefix."ts_booking_users( role_id, session_id, username, password, created)
					value(1, '".md5(uniqid(rand(), true))."','admin', 'admin123', now())";
		mysql_query( $sql );
