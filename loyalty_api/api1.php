...
//$_POST['user'] = 'symbol2'; 
//$_POST['pass'] = '010101'; 
//$_POST['action'] = 'getConsomers';//   
//$_POST['action'] = 'useStamp';// 'getCustomerInfo' ;//'getConsomers';//    'AuthenticateUser';//   
//$_POST['userid'] = 640;
//$_POST['stmpid'] = 7;
//$_POST['giftid'] = 1;
...
function getConsomers($str=''){
    ...
    {
        $where =" WHERE name LIKE '%". $str ."%' or lastname LIKE '%". $str ."%' or email LIKE '%". $str ."%' or mobile LIKE '%". $str ."%' or telephone LIKE '%". $str ."%' or city LIKE '%". $str ."%' or postalcode LIKE '%". $str ."%' or country LIKE '%". $str ."%' ";
    }
    $data = $wpdb -> get_results("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $prefix . sm_clients . $where . " order by name ASC");
...
function addconsumer($name,$email,$phone){
   if(empty($email)){
        ...
        $idclient = check_customer($email,$phone); 
        if($idclient){
           $client_id = $idclient;
        }else{
        ...
        if(!empty($email)){
            if(!email_exists( $email )) {
                ...
                return array('status'=>'success','data'=>array('id'=>$newuserID,'cardID'=>$idcard),'msg'=>'new');
            }else{
                $user = get_user_by( 'email', $email );
                $idcard = get_user_meta( $user->ID, 'hashcard', TRUE );
                if($idcard && !empty($idcard)){
                return array('status'=>'success','data'=>array('id'=>$user->ID,'cardID'=>$idcard),'msg'=>'existcard');
                }else{
                     $idcard = encrypt_str($user->ID);
                     update_user_meta( $user->ID, 'hashcard', $idcard );
                return array('status'=>'success','data'=>array('id'=>$user->ID,'cardID'=>$idcard),'msg'=>'newcard');
                }
            }  
        }
   }
}
function check_customer($email,$phone){
    global $wpdb;
    $prefix = get_prefix();
    $email = secure_jsonstr($email);
    $phone = secure_jsonstr($phone);
    if(empty($email) && !empty($phone)){
        $ckemail  =  $wpdb->get_var('SELECT id  FROM ' . $prefix . sm_clients . ' where mobile = "'.$phone.'"') ;
...
function api_use_stamps($userid,$stmpid){
    global $wpdb;
    $prefix = get_prefix();
    ...
    if(!api_checkuserstmp($userid,$stmpid)){
       $arr_data = array(
        ...
        return array('status'=>'failed','data'=>array('msg'=>'already used')); 
    }
}
...
function api_add_stamp($userid,$stmpid,$stampamount){
    global $wpdb;
    $prefix = get_prefix();
...
function api_add_points($userid,$points){
    global $wpdb;
    $userid = intval($userid);
...
function api_use_points($giftid,$userid){
    global $wpdb;
    $prefix = get_prefix();
    $idblog = get_prefix(TRUE);
...
function api_getcustomerinfo($userid){
    global $wpdb;
    $userid = intval($userid);
    $prefix = get_prefix();
    $idblog = get_prefix(TRUE);
    ...
    return array('status'=>'success','data'=>array('fname'=>$fname, 'lname'=>$fname, 'mobile'=>$mobile, 'email'=>$email, 'points' => $points, 'stamps'=> $stamps, 'stampshistory' => $stampshistory, 'pointshistory' => $pointshistory));
}
function api_get_usercardid($userid){           
