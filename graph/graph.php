...
// +----------------------------------------------------------------------+
// | Author: Rajath Kamal <rajath.k@armiasystems.com>
// | Modified : Meena Susan Joseph <meena.s@armiasystems.com>             |
// +----------------------------------------------------------------------+
...
class graph extends fusioncharts
{
	//Variable decleration
	private $loginId 		= '';
	private $loginKey		= '';
	public $blastArray		=array();
        public $magazinesql             ='';
        public $remindersql             = '';
        public $FD                      = '';
	public function __construct($graphType, $width =450, $height=250)
        {
            
            if($graphType==1)
            {
               # Create Column 3D + Line Dual Y-Axis Combination Chart
               $this->FD = new fusioncharts("MSLine", $width, $height, "","transparent");

               # Set the relative path of the swf file
               //$this->FD->setSWFPath(BASE_URL."public/FusionCharts/FusionCharts/");
               $this->FD->setSWFPath(BASE_URL."fusioncharts/FusionCharts/");


               # Store chart attributes in a variable
            //   $strParam="caption=User visit statistics; subcaption=; xAxisName=Month; yAxisMinValue=15000; yAxisName=Count; numberPrefix=; showNames=1; showValues=0; rotateNames=0; showColumnShadow=1; animation=1; showAlternateHGridColor=1; AlternateHGridColor=ff5904; divLineColor=ff5904; divLineAlpha=20; alternateHGridAlpha=5; canvasBorderColor=666666; baseFontColor=666666;decimalPrecision=0;";

               # Set chart attributes
               $this->FD->setChartParams($strParam);
            }else if ($graphType==9){
               

                # Create FusionCharts PHP Class object for single series MS column3d chart
                $this->FD = new FusionCharts("MSColumn3D", $width, $height);

                # Set Relative Path of chart SWF file.
                $this->FD->setSWFPath(BASE_URL."fusioncharts/FusionCharts/");
            }else if($graphType==8){
                 # Create FusionCharts PHP Class object for single series column3d chart
                 $this->FD = new FusionCharts("Column3D", $width, $height);

                   # Set Relative Path of chart SWF file.
                 $this->FD->setSWFPath(BASE_URL."fusioncharts/FusionCharts/");
            }else if($graphType==10){
                 $this->FD = new FusionCharts("Pie3D", $width, $height);
        ...
        public function setCategory($catVal)
        {           
            $this->FD->addCategory($catVal);
        }
        public function setAxis()
        {
            $this->FD->addDataset("User Visit Count","parentYaxis=S");
        }
        public function adddata($chartVal)
        {           
            $this->FD->addChartData($chartVal,"showValue=0;");
        }      
        ...
        public function setChartParams($caption = 'caption', $formatNumberScale = 1, $rotateValues = 1, $decimalPrecision = 1, $numvDivLines = 5 ,$showPercentValues=0, $xAxisName='Month', $yAxisName='Count',$showLegend=0,$canvasbgColor='FFFFFF',$divLineColor='B7D0E1',$canvasBorderColor='FFFFFF',$canvasBaseColor='',$yAxisMinValue=0,$yAxisMaxValue=30){
             # Define chart attributes
             $strParam="caption=$caption;formatNumberScale=$formatNumberScale;xAxisName=$xAxisName;yAxisName=$yAxisName;rotateValues=$rotateValues;decimalPrecision=$decimalPrecision;numDivLines=$numvDivLines;yAxisMinValue=$yAxisMinValue;yAxisMaxValue=$yAxisMaxValue;showPercentValues=$showPercentValues;showLegend=$showLegend;canvasbgColor=$canvasbgColor;divLineColor=$divLineColor;canvasBorderColor=$canvasBorderColor;canvasBaseColor=$canvasBaseColor;";
              # Set chart attributes
              $this->FD->setChartParams($strParam);
        }

        public function addChartData($arrData,$arrCatNames =NULL){
           
            $this->FD->addChartDataFromArray($arrData,$arrCatNames);
        }
