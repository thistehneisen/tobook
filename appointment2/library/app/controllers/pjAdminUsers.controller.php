...
class pjAdminUsers extends pjAdmin
{
	...
	public function pjActionDeleteUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_GET['id']) && $_GET['id'] != $this->getUserId())
			{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_POST['id']) && (int) $_POST['id'] > 0)
			{
				...
				if (!empty($arr))
				{
					...
					$pjUserModel->reset()->set('id', $_POST['id'])->modify(array('is_active' => $sql_status));
					pjAppController::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'User has been updated.'));
				}
				pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'User has not been found.'));
			}
