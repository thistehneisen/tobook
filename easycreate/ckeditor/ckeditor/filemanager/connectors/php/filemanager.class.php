...
//      $this->doc_root = $_SERVER['DOCUMENT_ROOT'];
      ...
      $this->doc_root = $_SESSION['imagesavepath']."/";
      $this->imagePath = $_SESSION['replacepath'];
      $this->filePath = $_SESSION['FILE_PATH'];
   ...
   $this->imagePath = $_SESSION['replacepath'];
   $this->filePath = $_SESSION['FILE_PATH'];
   $this->sysReplacePath = $this->imagePath;
   if($_SESSION['fromServer'] == 1)
      {
          $this->doc_root = $_SESSION['serverImagePath']."/";
          $this->imagePath = $_SESSION['serverReplacepath'].'/';
          $this->filePath = $_SESSION['serverFILE_PATH'];
      }
  ...
  public function getinfo() {
    ...
    if($_SESSION['fromServer'] == 1)
    {
        copy($this->imagePath.end($fileName),$_SESSION['imagesavepath']."/".end($fileName));
        chmod($_SESSION['imagesavepath']."/".end($fileName),$_SESSION['SERVER_PERMISSION']);
    }
    
    $array = array(
			'Path'=> $this->sysReplacePath.end($fileName),
			'Filename'=>$this->item['filename'],
			'File Type'=>$this->item['filetype'],
			'Preview'=>$this->filePath.'/'.end($fileName),
  ...
  public function getfolder() {
    ...
    $current_path = $this->doc_root;
    if(!is_dir($current_path)) {
      $this->error(sprintf($this->lang('DIRECTORY_NOT_EXIST'),$this->doc_root));
    }
    if(!$handle = opendir($current_path)) {
      $this->error(sprintf($this->lang('UNABLE_TO_OPEN_DIRECTORY'),$this->doc_root));
    } else {
      ...
      foreach($filesDir as $file) {
        if(is_dir($current_path . $file)) {
          if(!in_array($file, $this->config['unallowed_dirs'])) {
            $array[$this->filePath . $file.'/'] = array(
          ...
          $this->get_file_info($this->doc_root . $file);
  ...
  public function rename() {
    ...
    if(file_exists ($this->doc_root . $this->get['new'])) {
      if($suffix=='/' && is_dir($this->doc_root . $this->get['new'])) {
        $this->error(sprintf($this->lang('DIRECTORY_ALREADY_EXISTS'),$this->get['new']));
      }
      if($suffix=='' && is_file($this->doc_root . $this->get['new'])) {
        $this->error(sprintf($this->lang('FILE_ALREADY_EXISTS'),$this->get['new']));
      }
    }

    if(!rename($this->doc_root . $this->get['old'],$this->doc_root . $this->get['new'])) {
  ...
  public function delete() {
    ...
    if(is_dir($this->doc_root)) {
      $this->unlinkRecursive($this->doc_root);
      $array = array(
				'Error'=>"",
				'Code'=>0,
				'Path'=>$this->doc_root
      ...
      return $array;
    } else if(file_exists($this->doc_root)) {
      unlink($this->doc_root);
      $array = array(
				'Error'=>"",
				'Code'=>0,
				'Path'=>$this->doc_root
  ...
  public function add() {
    ...
    if(!$this->config['upload']['overwrite']) {
      $_FILES['newfile']['name'] = $this->checkFilename($this->doc_root,$_FILES['newfile']['name']);
    }
    move_uploaded_file($_FILES['newfile']['tmp_name'], $this->doc_root . $_FILES['newfile']['name']);
  ...
  public function download() {
    if(isset($this->doc_root) && file_exists($this->doc_root)) {
      header("Content-type: application/force-download");
      header('Content-Disposition: inline; filename="' . basename(rawurldecode($this->doc_root)) . '"');
      header("Content-Transfer-Encoding: Binary");
      header("Content-length: ".filesize($this->doc_root));
      header('Content-Type: application/octet-stream');
      header('Content-Disposition: attachment; filename="' . basename(rawurldecode($this->doc_root)) . '"');
      readfile($this->doc_root . $this->doc_root);
    } else {
      $this->error(sprintf($this->lang('FILE_DOES_NOT_EXIST'),rawurldecode($this->doc_root)));
    }
  }
...
if(isset($this->get['path']) && file_exists($this->doc_root . rawurldecode($this->get['path']))) {
  ...
  private function get_file_info($path='',$return=array()) {
    if($path=='') {
      $path = rawurldecode($this->doc_root);
    }
    $tmp = explode('/',$path);
    $this->item['filename'] = $tmp[(sizeof($tmp)-1)];
//    $path=$this->filePath.$this->item['filename'];
    ...
    if(is_dir( $path)) {
      ...
      $this->item['preview'] = 'connectors/php/filemanager.php?mode=preview&path=' . rawurlencode($this->item['filename']);
