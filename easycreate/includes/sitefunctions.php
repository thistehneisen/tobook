...
function saveSiteDetails($site_id=0) {
    ...
    if(mysql_num_rows($getSiteRes) > 0) {
        $sqlSiteMaster =  "UPDATE tbl_site_mast SET vsite_name='".mysql_real_escape_string($siteDetails['siteInfo']["siteName"])."',nuser_id='".$_SESSION["session_userid"]."',
                           ...
                           vcolor='".mysql_real_escape_string($siteDetails['siteInfo']["stclr"])."',vcaption='".mysql_real_escape_string($siteDetails['siteInfo']["captionname"])."' ,vcaption_style='".mysql_real_escape_string($captionStyle)."', google_analytics_code='".mysql_real_escape_string($siteDetails['siteInfo']["site_google_analytics_code"])."' WHERE nsite_id=".$site_id;
        ...
        if($siteDetails['siteInfo']){
            $sqlSiteMaster = "INSERT INTO tbl_site_mast (vsite_name,nuser_id,ncat_id,ntemplate_id,ntheme_id,vtype,vtitle,
                              vmeta_description,vmeta_key,vlogo,vlogo_name,vcompany,vcompany_style,vcolor,vcaption,vcaption_style,google_analytics_code,ddate) values (
