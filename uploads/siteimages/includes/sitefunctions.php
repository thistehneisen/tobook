...
function getSiteDataSession($siteId) {
    ...
    $_SESSION['siteDetails']['siteInfo'] = array( 'logooption'   => $siteData['vlogo'],
			...
			'background-image'  => $siteData['vimage'],
...
function saveSiteDetails($site_id=0) {
	...
	$siteBackImage2 = ($siteDetails['siteInfo']["background-image"])?$siteDetails['siteInfo']["background-image"]:$getSiteVal['backimage'];
    ...
    if(mysql_num_rows($getSiteRes) > 0) {
        ...
        if($siteDetails['siteInfo']){
            ...
            $sqlSiteMaster = "INSERT INTO tbl_site_mast (vsite_name,nuser_id,ncat_id,ntemplate_id,ntheme_id,vtype,vtitle,
							  ...
							  '".mysql_real_escape_string($siteDetails['siteInfo']["background-image"])."',
...
function applySiteColor($siteColor, $siteBackImage234) {
    ...
    if(isset($siteColorTag)) {
		...
		if ($siteBackImage234 != '')
		{
			$siteColorTag->setAttribute("style","background-image:url(uploads/siteimages/".$siteBackImage234.")");
		}
